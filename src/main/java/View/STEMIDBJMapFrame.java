...
/*
 *    EDITED VERSION OF JMapFrame FROM GeoTools FOR USE BY STEMInteractiveDatabase SOFTWARE
 *      - Changed default close operation to dispose (previously exit, resulting in ALL windows closing)
 *      - Added option to display infoPanel
 *          - functions setInfoPanel() and enableInfoPanel()
*/
 ...
 * @version $Id$
 */
public class STEMIDBJMapFrame extends JFrame {
    ...
    private static void doShowMap(MapContent content) {
        final STEMIDBJMapFrame frame = new STEMIDBJMapFrame(content);
     ...
     * @param content the map content
     */
    public STEMIDBJMapFrame(MapContent content) {
    ...
    public void initComponents() {
        ...
        JPanel panel = new JPanel(new BorderLayout());  // NEW LAYOUT VERSION
        ...
        if (showStatusBar) {
            panel.add(JMapStatusBar.createDefaultStatusBar(mapPane), BorderLayout.SOUTH);
        }
    ...
    public void enableInfoPanel(boolean bool)
    {
        // If infoPanel was not set, assign false to showInfoPanel
